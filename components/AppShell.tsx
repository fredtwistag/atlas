    ...
    <div className="min-h-screen bg-bg lg:grid lg:grid-cols-[248px_1fr]">
      ...
      >
        <button
          ...
          className="grid h-[44px] w-[44px] place-items-center rounded-sm text-text-2 hover:bg-surface-2 hover:text-text"
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          >
            <button
              onClick={() => setDrawerOpen(false)}
              className="absolute right-3 top-4 z-10 grid h-[44px] w-[44px] place-items-center rounded-sm text-text-3 hover:bg-surface-2 hover:text-text"
              aria-label="Close"
            >
              <X className="h-4 w-4" />
            </button>
